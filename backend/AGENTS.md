...
28. **LoopDetectionMiddleware** - *(optional, if `loop_detection.enabled`)* Detects repeated tool-call loops; hard-stop clears both structured `tool_calls` and raw provider tool-call metadata before forcing a final text answer; stamps `loop_capped` via `consume_stop_reason` (#3875 Phase 2), symmetric to `TokenBudgetMiddleware`. Records a `middleware:loop_detection` audit event (action=`warn`|`cap`) in `run_events` so a loop fire is observable alongside the stop reason (#4243)
